...
def main():
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        # train the network
        scores = train(train_loader, model, optimizer, criterion, epoch, lr_schedule, writer)
...
def train(train_loader, model, optimizer, criterion, epoch, lr_schedule, writer):
    ...
    for it, (inputs, labels) in enumerate(train_loader):
        ...
        # print(f'input mean {inputs.mean()}')
    ...
    writer.add_scalar('Loss_3/losses', losses.avg, global_step=epoch)
    writer.add_scalar('Loss_3/lr', optimizer.param_groups[-1]['lr'], global_step=epoch)
