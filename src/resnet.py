...
class Bottleneck(nn.Module):
    ...
    def __init__(
        ...
        self.att = GAM_Attention(planes * self.expansion, planes * self.expansion)
        self.relu = nn.ReLU()
        self.downsample = downsample
        self.stride = stride
...
class ResNet(nn.Module):
    ...
    def execute_backbone(self, x, avgpool=True):
        ...
        if avgpool:
            ...
            x = jt.flatten(x, 1)
            return x
        # [32,512,7,7] [32,356,14,14] [32,128,28,28] [32,64,56,56]
        return x, c3, c2, c1
    ...
    def execute(self, inputs, gridq=None, gridk=None, mode='train'):
                ...
                if self.train_mode == 'pixelattn':
                    ...
                    if self.shallow is not None:
                        ...
                        for i, stage in enumerate(self.shallow):
                            _c = _stages[stage - 1]
                            _out_c = self.projection_head_pixel_shallow.layers[f'projection_head_pixel{stage}'](_c)
        ...
        if self.shallow is not None:
            for i, stage in enumerate(self.shallow):
                embedding_c_, output_c_ = self.execute_head_shallow(output_c[i], stage=stage)
...
if __name__ == '__main__':
    ...
    model = resnet18(train_mode='pretrain')
    inp = jittor.ones(128, 3, 224, 224),
