...
from jimm.data import IMAGENET_DEFAULT_MEAN, IMAGENET_DEFAULT_STD
from jimm.models.helpers import named_apply, build_model_with_cfg, checkpoint_seq
from jimm.models.layers import trunc_normal_, SelectAdaptivePool2d, DropPath, Mlp, LayerNorm2d, \
    create_conv2d, make_divisible
from jimm.models.layers.mlp import ConvMlp
from jimm.models.layers.padding import get_padding
from src.rfconv import RFConv2d
import os
...
class RFConvNeXtBlock(jittor.Module):
    """ ConvNeXt Block
    ...
    def __init__(
        ...
        self.gamma = ls_init_value * jittor.ones(dim_out) if ls_init_value > 0 else None
...
class RFConvNeXt(nn.Module):
    ...
    def __init__(
        ...
        dp_rates = [x.tolist() for x in jittor.misc.linspace(0, drop_path_rate, sum(depths)).split(depths)]
        stages = []
        prev_chs = dims[0]
        ...
        self.head = nn.Sequential(OrderedDict([
            ...
            ('norm', nn.Identity() if head_norm_first else LayerNorm2d(self.num_features)),
    ...
    def prepare_rfsearch(self, pretrained_weights, rf_mode, kernel_cfgs, search_cfgs):
        ...
        if pretrained_weights is None or not os.path.exists(pretrained_weights):
            ...
            checkpoint = jittor.load(pretrained_weights)
        ...
        #             padding=(get_padding(kernel[0], module.stride[0], module.dilation[0]),
...
def _init_weights(module, name=None, head_init_scale=1.0):
    if isinstance(module, nn.Conv2d):
        ...
        if name and 'head.' in name:
            module.weight.data.__mul__(head_init_scale)
            module.bias.data.__mul__(head_init_scale)
