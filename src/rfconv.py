...
from jimm.models.layers.padding import get_padding
...
class RFConv2d(nn.Conv2d):
    ...
    def __init__(self,
        ...
        if pretrained is not None and rf_mode == 'rfmerge':
            ...
            new_weight = jittor.zeros((old_weight.shape[0], old_weight.shape[1], new_kernel_size[0], new_kernel_size[1]))
            ...
            pretrained['rates'][0] = jittor.Var([1, 1])
            pretrained['num_rates'] = jittor.Var([1])
        ...
        self.sample_weights = jittor.rand(self.num_branches)
        self.register_buffer('counter', jittor.zeros(1))
        self.register_buffer('current_search_step', jittor.zeros(1))
        self.register_buffer('rates', jittor.ones((self.num_branches, 2), dtype='int32'))
        self.register_buffer('num_rates', jittor.ones(1, dtype='int32'))
        self.rates[0] = jittor.Var([self.dilation[0], self.dilation[1]])
        self.sample_weights.data.fill(self.init_weight)
    ...
    def _conv_forward_dilation(self, input, dilation_rate):
        if self.padding_mode != 'zeros':
            return jittor.nn.conv2d(jittor.nn.pad(input, self._reversed_padding_repeated_twice, mode=self.padding_mode),
                                    self.weight, self.bias, self.stride,
                                    _pair(0), dilation_rate, self.groups)
        else:
            padding = (
                dilation_rate[0] * (self.kernel_size[0] - 1) // 2, dilation_rate[1] * (self.kernel_size[1] - 1) // 2)
            return jittor.nn.conv2d(input, self.weight, self.bias, self.stride,
                                    padding, dilation_rate, self.groups)

    def normlize(self, w):
        abs_w = jittor.abs(w)
        norm_w = abs_w / jittor.sum(abs_w)
        return norm_w
    ...
    def expand(self):
        ...
        for i, rate in enumerate(rates):
            self.rates[i] = jittor.Var([rate[0], rate[1]])
...
if __name__ == '__main__':
    layer = RFConv2d(in_channels=3, out_channels=3)
    inp = jittor.random((8, 3, 256, 256))
    out_put = layer(inp)
    print(out_put)
