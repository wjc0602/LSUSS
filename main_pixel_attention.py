...
def train(train_loader, model, optimizer, epoch, lr_schedule, queue, writer):
    ...
    for it, inputs in enumerate(train_loader):
        ...
        with jt.no_grad():
            model.prototypes.weight.assign(model.prototypes.weight.normalize(dim=1, p=2))
    ...
    if jt.rank == 0:
        writer.add_scalar('Loss_2/losses', losses.avg, global_step=epoch)
        writer.add_scalar('Loss_2/lr', optimizer.param_groups[0]["lr"], global_step=epoch)
