...
from src.utils import (initialize_exp, restart_from_checkpoint, fix_random_seeds, AverageMeter, distributed_sinkhorn)
import math
import os
import shutil
import time
...
def main():
    ...
    cosine_lr_schedule = \
        np.array([args.final_lr + 0.5 * (args.base_lr - args.final_lr) * (
                1 +
                math.cos(
                    math.pi * t / (len(train_loader) * (args.epochs - args.warmup_epochs))
                )
        )
                  for t in iters])
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        if args.queue_length > 0 and epoch >= args.epoch_queue_starts and queue is None:
            queue = jt.zeros((len(args.crops_for_assign), args.queue_length // jt.world_size, args.feat_dim))

        # train the network
        scores, queue = train(train_loader, model, optimizer, epoch, lr_schedule, queue, writer)
        training_stats.update(scores)

        # save checkpoints
        if jt.rank == 0:
            save_dict = {"epoch": epoch + 1, "state_dict": model.state_dict(), "optimizer": optimizer.state_dict(), }
            jt.save(save_dict, os.path.join(args.dump_path, "checkpoint.pth.tar"), )
            if epoch % args.checkpoint_freq == 0 or epoch == args.epochs - 1:
                shutil.copyfile(os.path.join(args.dump_path, "checkpoint.pth.tar"), os.path.join(args.dump_checkpoints, "ckp-" + str(epoch) + ".pth.tar"), )
...
def train(train_loader, model, optimizer, epoch, lr_schedule, queue, writer):
    ...
    for it, (inputs, gridq, gridk) in enumerate(train_loader):
        ...
        (embedding, output, embedding_deep_pixel, output_deep_pixels,) = model(inputs, gridq=gridq, gridk=gridk)
        embedding = embedding.detach()
        bs = inputs[0].size(0)

        # ============ swav loss ... ============
        loss_i2i, labels, queue, use_the_queue = swav_loss(args, model, embedding, output, queue, use_the_queue, bs)
        loss_d2s = d2s_loss(args, output[bs * np.sum(args.nmb_crops):], labels, bs, shallow=args.shallow)
        loss_p2p = p2p_loss(args, output_deep_pixels, embedding_deep_pixel, )
        loss = jt.sum(jt.stack([loss_i2i * args.weights[0]] + [x * args.weights[1 + i] for i, x in enumerate(loss_d2s)], dim=0)) / sum(args.weights) + loss_p2p
    ...
    writer.add_scalar('Loss/loss_i2i', losses.avg, global_step=epoch)
    writer.add_scalar('Loss/loss_p2p', losses_p2p.avg, global_step=epoch)
    writer.add_scalar('Loss/loss_d2s', losses_d2s.avg, global_step=epoch)
    writer.add_scalar('Loss/lr', optimizer.param_groups[0]["lr"], global_step=epoch)
    ...
    if shallow is not None:
        for stage in shallow:
            ...
            for i, crop_id in enumerate(args.crops_for_assign):
                ...
                # cluster assignment prediction
                subloss = 0
                for v in np.delete(np.arange(2 * (3 - stage), 2 * (3 - stage) + 2), crop_id, ):
                    x = output[bs * v: bs * (v + 1)] / args.temperature
                    subloss -= jt.mean(jt.sum(q * jt.nn.log_softmax(x, dim=1), dim=1))
